...
class AmericanExpress:
    ...
    def  Verifier_numero_carte(self,numeros):
        ...
        if len(numeros) != 16 and len(numeros) != 17:

            exit("La Longueur du numéro de la carte ne doit pas ni superieur ni inferieur à 16 et 17 !")
        ...
        if reste == 0:
            ...
            if num_length>=16 and num_length<=17:
               if ''.join(numeros[:2]) in strings_entre(30,39):
                  print("elle est aussi une carte American Express")

               else: print("Mais elle n'est pas une carte American Express")
        else:
...
Amex.Verifier_numero_carte("4024007146302608")
